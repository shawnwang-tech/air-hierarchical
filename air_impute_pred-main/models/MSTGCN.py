...
class MSTGCN_block(nn.Module):
    ...
    def __init__(self, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, graph, device):
        super(MSTGCN_block, self).__init__()
        self.cheb_conv = cheb_conv(K, graph, in_channels, nb_chev_filter, device)
...
class MSTGCN(nn.Module):
    ...
    def __init__(self, gpu_id, batch_size, in_dim, out_dim, use_met, use_time, enc_len, dec_len, graph, node_num):
        ...
        self.BlockList = nn.ModuleList([MSTGCN_block(in_channels, K, nb_chev_filter, nb_time_filter, time_strides, self.graph, device)])

        self.BlockList.extend([MSTGCN_block(nb_time_filter, K, nb_chev_filter, nb_time_filter, 1, self.graph, device) for _ in range(nb_block-1)])
