...
class mtgru_cell_list(nn.Module):
    
    def __init__(self, k_hop, input_dim, hid_dim, interval_set):
        super(mtgru_cell_list, self).__init__()
        layers = [MTGRUCell(input_dim, hid_dim, interval_set) for _ in range(k_hop)]
        self.cell_list = nn.ModuleList(layers)
    
    def forward(self, x_gru, hn, idx, deep):
        return self.cell_list[deep](x_gru, hn, idx)
...
class MTMSGRU(nn.Module):
    ...
    def __init__(self, hist_len, pred_len, k_hop, batch_size, num_node,
        ...
        # self.mtgru_cell_set = mtgru_cell_list(self.k_hop, input_dim, hid_dim, interval_set)
        # self.mtgru_cell_set = []
        # for _ in range(self.k_hop):
        #     self.mtgru_cell_set.append(MTGRUCell(input_dim, hid_dim, interval_set).to(device)) # train_multihist_MTMS中model.to(device)无效, 应该是使用list的缘故 -> 手动to(device)
        # self.mtgru_cell = MTGRUCell(input_dim, hid_dim, interval_set) # train_multihist_MTMS中model.to(device)有效
        # self.mtgru_squence = nn.Sequential(MTGRUCell(input_dim, hid_dim, interval_set),
        #                                    )
...
class MTMSGNN(nn.Module):
    def __init__(self, gpu_id, hist_len, pred_len, in_dim, out_dim, hid_dim, node_num, batch_size, edge_index, edge_attr, wind_mean, wind_std, use_met, use_time, interval_set, k_hop):
        ...
        self.MTMSGRU = MTMSGRU(self.hist_len, self.pred_len, self.k_hop, self.batch_size, self.node_num, self.in_dim + self.gnn_out, self.hid_dim, self.interval_set, self.graph_gnn, self.fc_out, self.device, use_met, use_time)

    def forward(self, model_input): 
        y_pred = self.MTMSGRU(model_input)
