...
class MTMSGRU(nn.Module):
    ...
    def __init__(self, hist_len, pred_len, k_hop, batch_size, num_node, city_num, 
                 ...
                 fc_out, c_fc_out, fc_u, fc_u2origin, c2s, s2c, afc_s2c,
                 device,
                 use_met, use_time):
        ...
        self.mtgru_cell_decoder_set = nn.ModuleList([GRUCell(input_dim, hid_dim) for _ in range(k_hop)])
    ...
    def forward(self, model_input):
        ...
        for t in range(1, self.hist_len):
            # city_u_t = torch.bmm(afc, city_u_set[:, t])
...
class HighAirv2(nn.Module):
    def __init__(self, gpu_id, hist_len, pred_len, in_dim, out_dim, hid_dim, node_num, city_num, afc_s2c, batch_size, edge_index, edge_attr, c_edge_index, c_edge_attr, wind_mean, wind_std, c_wind_mean, c_wind_std, use_met, use_time, interval_set, k_hop):
        ...
        self.MTMSGRU = MTMSGRU(self.hist_len, self.pred_len, self.k_hop, self.batch_size, self.node_num, self.city_num, self.in_dim + self.gnn_out, self.hid_dim, self.interval_set, self.graph_gnn, self.c_graph_gnn, self.fc_out, self.c_fc_out, self.fc_u, self.fc_u2origin, self.c2s, self.s2c, self.afc_s2c, self.device, use_met, use_time)
