...
class MTMSGRU(nn.Module):
    ...
    def __init__(self, hist_len, pred_len, k_hop, batch_size, num_node, city_num, 
                 input_dim, gnn_out_dim, hid_dim, interval_set, 
        ...
        self.mtgru_cell_set = nn.ModuleList([GRUCell(input_dim + gnn_out_dim, hid_dim) for _ in range(k_hop)])
        self.c_mtgru_cell_set = nn.ModuleList([GRUCell(input_dim + gnn_out_dim, hid_dim) for _ in range(k_hop)])
        self.x2gnn = nn.Linear(input_dim, input_dim + gnn_out_dim)
...
class HighAirv2(nn.Module):
    def __init__(self, gpu_id, hist_len, pred_len, in_dim, out_dim, hid_dim, gnn_out_dim, node_num, city_num, afc_s2c, batch_size, edge_index, edge_attr, c_edge_index, c_edge_attr, wind_mean, wind_std, c_wind_mean, c_wind_std, use_met, use_time, interval_set, k_hop):
        ...
        self.MTMSGRU = MTMSGRU(self.hist_len, self.pred_len, self.k_hop, self.batch_size, self.node_num, self.city_num, self.in_dim, self.gnn_out, self.hid_dim, self.interval_set, self.graph_gnn, self.c_graph_gnn, self.fc_out, self.c_fc_out, self.c2s, self.s2c, self.afc_s2c, self.device, use_met, use_time)
