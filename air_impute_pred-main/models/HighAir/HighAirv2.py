...
class MTMSGRU(nn.Module):
    ...
    def __init__(self, hist_len, pred_len, k_hop, batch_size, num_node, city_num, 
                 ...
                 fc_out, c_fc_out, fc_u, fc_u2origin, fc_u2gnn, c2s, s2c, afc_s2c,
                 device,
                 use_met, use_time):
        ...
        # self.fc_u = fc_u
        # self.fc_u2origin = fc_u2origin
        self.fc_u2gnn = fc_u2gnn
        ...
        self.mtgru_cell_set = nn.ModuleList([GRUCell(input_dim + hid_dim, hid_dim) for _ in range(k_hop)])
        self.mtgru_cell_decoder_set = nn.ModuleList([GRUCell(12, hid_dim) for _ in range(k_hop)])
    ...
    def forward(self, model_input):
        ...
        for t in range(1, self.hist_len):
            xcn = torch.cat([c_x_hist[:, t-1], c_features[:, t]], dim=-1)
            xcn_gnn = xcn
            ...
            for k in range(self.k_hop):        
                ...
                # city_u = self.fc_u(hcn_set[k])
                city_u = hcn_set[k]
                
            city_u_set.append(city_u.reshape(self.batch_size, self.city_num, -1))

        city_u_set = torch.stack(city_u_set, dim=1)

        afc = torch.FloatTensor(self.afc_s2c[None, :, :]).repeat(self.batch_size, 1, 1).to(self.device)

        for t in range(1, self.hist_len):
            city_u_t = self.fc_u2gnn(torch.bmm(afc, city_u_set[:, t-1]))
            xn = torch.cat([x_hist[:, t-1], features[:, t], city_u_t], dim=-1)
            # xn = self.fc_u2origin(xn)
        ...
        for i in range(self.pred_len):
            ...
            for k in range(self.k_hop):
                ...
                ### Highair在pred length中不做gnn
                x_gru = xn_gnn
                hn_set[k] = self.mtgru_cell_decoder_set[k](x_gru, hn_set[k])
...
class HighAirv2(nn.Module):
    def __init__(self, gpu_id, hist_len, pred_len, in_dim, out_dim, hid_dim, node_num, city_num, afc_s2c, batch_size, edge_index, edge_attr, c_edge_index, c_edge_attr, wind_mean, wind_std, c_wind_mean, c_wind_std, use_met, use_time, interval_set, k_hop):
        ...
        self.graph_gnn = GraphGNN(self.device, edge_index, edge_attr, self.in_dim + self.hid_dim, self.gnn_out, wind_mean, wind_std)
        ...
        self.fc_u = nn.Linear(self.hid_dim, 8)
        self.fc_u2origin = nn.Linear(self.in_dim + 8, self.in_dim)
        self.fc_u2gnn = nn.Linear(self.hid_dim, self.hid_dim)

        self.c2s = nn.Linear(in_dim, in_dim)
        self.s2c = nn.Linear(in_dim, in_dim)

        self.MTMSGRU = MTMSGRU(self.hist_len, self.pred_len, self.k_hop, self.batch_size, self.node_num, self.city_num, self.in_dim + self.gnn_out, self.hid_dim, self.interval_set, self.graph_gnn, self.c_graph_gnn, self.fc_out, self.c_fc_out, self.fc_u, self.fc_u2origin, self.fc_u2gnn, self.c2s, self.s2c, self.afc_s2c, self.device, use_met, use_time)
