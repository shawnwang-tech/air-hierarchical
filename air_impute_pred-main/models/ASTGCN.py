...
class cheb_conv_withSAt(nn.Module):
    ...
    def __init__(self, K, graph, in_channels, out_channels, DEVICE):
        '''
        :param K: int
...
class ASTGCN_block(nn.Module):
    ...
    def __init__(self, DEVICE, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, graph, num_of_vertices, num_of_timesteps):
        super(ASTGCN_block, self).__init__()
        self.TAt = Temporal_Attention_layer(DEVICE, in_channels, num_of_vertices, num_of_timesteps)
        self.SAt = Spatial_Attention_layer(DEVICE, in_channels, num_of_vertices, num_of_timesteps)
        self.cheb_conv_SAt = cheb_conv_withSAt(K, graph, in_channels, nb_chev_filter, DEVICE)
...
class ASTGCN(nn.Module):
    ...
    # def __init__(self, DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, num_for_predict, len_input, num_of_vertices):
        ...
        self.BlockList = nn.ModuleList([ASTGCN_block(DEVICE, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, self.graph, num_of_vertices, len_input)])

        self.BlockList.extend([ASTGCN_block(DEVICE, nb_time_filter, K, nb_chev_filter, nb_time_filter, 1, self.graph, num_of_vertices, len_input//time_strides) for _ in range(nb_block-1)])
