...
class MTMSGRU(nn.Module):
    ...
    def __init__(self, hist_len, pred_len, k_hop, batch_size, num_node, city_num, 
                 ...
                 fc_out, c_fc_out, c2s, s2c, c2s_h, s2c_h, afc_s2c,
                 device,
                 use_met, use_time):
    ...
    def forward(self, model_input):
        ...
        for t in range(self.hist_len-1):
            ...
            for k in range(self.k_hop):
                ...
                hn_set[k] = hn_set[k].view(self.batch_size, self.num_node, -1)
                hcn_set[k] = hcn_set[k].view(self.batch_size, self.city_num, -1)
                hn_set_tmp = hn_set[k]
                hn_set[k] = 0.5 * hn_set[k] + 0.5 * self.c2s_h(torch.bmm(torch.FloatTensor(self.afc_s2c[None, :, :]).repeat(self.batch_size, 1, 1).to(self.device), hcn_set[k]))
                hcn_set[k] = 0.5 * hcn_set[k] + 0.5 * self.s2c_h(torch.bmm(torch.FloatTensor(self.afc_s2c.transpose(1,0)[None, :, :]).repeat(self.batch_size, 1, 1).to(self.device), hn_set_tmp))
                hn_set[k] = hn_set[k].view(self.batch_size*self.num_node, -1)
                hcn_set[k] = hcn_set[k].view(self.batch_size*self.city_num, -1)
...
class MTCityGNNv2(nn.Module):
    def __init__(self, gpu_id, hist_len, pred_len, in_dim, out_dim, hid_dim, node_num, city_num, afc_s2c, batch_size, edge_index, edge_attr, c_edge_index, c_edge_attr, wind_mean, wind_std, c_wind_mean, c_wind_std, use_met, use_time, interval_set, k_hop):
        super(MTCityGNNv2, self).__init__()
        ...
        self.MTMSGRU = MTMSGRU(self.hist_len, self.pred_len, self.k_hop, self.batch_size, self.node_num, self.city_num, self.in_dim + self.gnn_out, self.hid_dim, self.interval_set, self.graph_gnn, self.c_graph_gnn, self.fc_out, self.c_fc_out, self.c2s, self.s2c, self.c2s_h, self.s2c_h, self.afc_s2c, self.device, use_met, use_time)
