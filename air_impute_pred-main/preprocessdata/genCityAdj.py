...
lat = np.array(df_c2s['lat'])
lon = np.array(df_c2s['lon'])
coordinates_array = np.stack((lat, lon), axis=1)

dist_array = pdist(coordinates_array)
dist_mx = squareform(dist_array)

for i in range(50):
    m = np.random.randint(0,41)
    n = np.random.randint(0,41)
    coord_m = coordinates_array[m,]
    coord_n = coordinates_array[n,]
    dist = geodesic(coord_m, coord_n).kilometers
    dist_scipy = pdist(np.stack((coord_m, coord_n), axis=0))
    dist_mx[m, n]
'''
df_c2s = pd.read_pickle('/home/tokaka22/Multi_scale_GNN/data/knn-data/hb_city_41_station_list.pkl')
lat = np.array(df_c2s['lat'])
...
# Sets entries that lower than a threshold, i.e., k, to zero for sparsity.
adj_mx[adj_mx < normalized_k] = 0
np.savetxt("adj_c.csv", adj_mx, delimiter=',')
np.save("adj_c.npy", adj_mx)
print("ok")
